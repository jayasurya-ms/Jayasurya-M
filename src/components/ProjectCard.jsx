  ...
  return (
    <div ref={cardRef} className="w-full">
      <div ref={animContainerRef} className="w-full opacity-0">
        <motion.div
          className={`flex flex-col ${isEven ? "lg:flex-row" : "lg:flex-row-reverse"} gap-3 lg:gap-16 items-center group cyber-glass-card p-6 md:p-8 lg:p-12 rounded-[2rem] w-full`}
          whileHover={{ y: -5 }}
        ...
        >
          ...
          >
            <span className="text-purple-400 text-[12px] md:text-sm tracking-widest uppercase mb-4 block font-medium">
              {project.category}
            </span>
            <h3 className="text-2xl md:text-4xl font-semibold tracking-tight text-white mb-6 group-hover:text-purple-300 transition-colors">
              {project.title}
            </h3>
            <p className="text-white/60 text-[13px] md:text-md font-light leading-relaxed mb-8 max-w-3xl whitespace-pre-line">
              {project.description}
            </p>
            ...
            <div className="flex flex-wrap gap-3 mb-6 md:mb-10">
              {project.tech_stack.map((tech, idx) => (
                <span
                  key={idx}
                  className="px-3 py-1 bg-purple-950/20 backdrop-blur-sm border border-purple-500/20 rounded-full text-purple-300 text-[12px] md:text-sm font-medium tracking-wide"
                >
                  {tech}
                </span>
              ))}
